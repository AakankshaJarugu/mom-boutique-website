/* ============================================
   HONEY'S FASHION - LUXURY INDIAN BOUTIQUE
   Splash Screen & Interactive Effects
   ============================================ */

// Create Sparkle Particles for Splash Screen
function createSplashSparkles() {
    const sparklesContainer = document.getElementById('splashSparkles');
    if (!sparklesContainer) return;

    const brandName = document.getElementById('splashBrand');
    if (!brandName) return;
...
function handleSplashTransition() {
    ...
    // Wait 2.5 seconds for splash animation to complete
    setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("Honey's Fashion - Luxury Indian Boutique website loaded!");
});
...
// Observe all sections and cards (after splash)
setTimeout(() => {
    const animatedElements = document.querySelectorAll(
        '.category-card, .product-card, .testimonial-card, .contact-info, .contact-form-container'
    );
...
if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
        ...
        const whatsappMessage = `Hello! I'm interested in Honey's Fashion collection.\n\nName: ${name}\nEmail: ${email}\nPhone: ${phone}\nMessage: ${message}`;
